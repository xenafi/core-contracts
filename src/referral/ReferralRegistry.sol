...
contract ReferralRegistry is Initializable, OwnableUpgradeable {
    ...
    function setReferrer(address _trader, address _referrer) external {
        ...
        if (
            _trader != address(0) && _referrer != address(0) && _trader != _referrer
                && referredBy[_trader] == address(0)
        ) {
            referredBy[_trader] = _referrer;
            referredSetTime[_trader] = block.timestamp;
            emit ReferrerSet(_trader, _referrer, block.timestamp);
        }
    }
    ...
    // ===============  EVENTS ===============
    event ReferrerSet(address indexed trader, address indexed referrer, uint256 timestamp);
