...
/// @title XenaMaster V2
contract XenaMasterV2 is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    using SafeERC20 for IERC20;
    using SafeERC20 for IWETH;

    uint256 private constant MAX_REWARD_PER_SECOND = 100 ether;
    uint256 private constant MAX_ALLOCT_POINT = 1e6;
    IPool public constant xenaPool = IPool(0x32B7bF19cb8b95C27E644183837813d4b595dcc6);
    IWETH public constant weth = IWETH(0x82aF49447D8a07e3bd95BD0d56f35241523fBab1);
    IERC20 public constant rewardToken = IERC20(0x6810AB468fFD38Accc787D0119dc20Ba1C9E554F);
    ...
    /// @param _rewardPerSecond The amount of XEN to be distributed per second.
    ...
    /// @return pending XEN reward for a given user.
        ...
        nonReentrant
    {
        ...
        IERC20(assetToken).safeIncreaseAllowance(address(xenaPool), assetAmount);
        xenaPool.addLiquidity(tranche, assetToken, assetAmount, minLpAmount, address(this));
    ...
    function addLiquidityETH(uint256 pid, uint256 minLpAmount, address to) external payable nonReentrant {
        ...
        weth.safeIncreaseAllowance(address(xenaPool), _amountIn);
        xenaPool.addLiquidity(tranche, address(weth), _amountIn, minLpAmount, address(this));
        uint256 lpAmount = ILPToken(tranche).balanceOf(address(this)) - balanceLpTokenBefore;
        ...
        nonReentrant
    {
        ...
        tranche.safeIncreaseAllowance(address(xenaPool), lpAmount);
        xenaPool.removeLiquidity(address(tranche), toToken, lpAmount, minOut, to);
    }
    ...
    function removeLiquidityETH(uint256 pid, uint256 lpAmount, uint256 minOut, address to) external nonReentrant {
        ...
        xenaPool.removeLiquidity(address(tranche), address(weth), lpAmount, minOut, address(this));
