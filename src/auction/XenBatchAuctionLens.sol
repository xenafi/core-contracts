...
contract XenBatchAuctionLens {
    ...
    // FOR XEN
    function getXenAuctionInfo(IBatchAuction _batchAuction) public view returns (AuctionInfo memory) {
    ...
    function getXenAuctionInfos(IAuctionFactory _auctionFactory, uint256 _skip, uint256 _take)
        external
        view
        returns (AuctionInfo[] memory _auctionInfos)
    {
        ...
        for (uint256 i = 0; i < _take; i++) {
            _auctionInfos[i] = getXenAuctionInfo(IBatchAuction(_auctions[_skip + i]));
        }
    }

    function getOnGoingXenAuctionInfos(IAuctionFactory _auctionFactory)
        external
        view
        returns (AuctionInfo[] memory _auctionInfos)
    {
        ...
        if (_totalAuctions > 0) {
            ...
            for (uint256 i = 0; i < _totalAuctions; i++) {
                AuctionInfo memory _batchAuction = getXenAuctionInfo(IBatchAuction(_auctions[i]));
                if (
    ...
    function getUpCommingXenAuctionInfos(IAuctionFactory _auctionFactory)
        external
        view
        returns (AuctionInfo[] memory _auctionInfos)
    {
    ...
    function getXenAuctionsCommittedByUser(IAuctionFactory _auctionFactory, address _user)
        external
        view
