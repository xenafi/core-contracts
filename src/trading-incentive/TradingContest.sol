...
import {IXENStaking} from "src/interfaces/IXENStaking.sol";
...
contract TradingContest is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, ITradingContest {
    ...
    function claimRewards(uint64 _batchId, address _to) external nonReentrant {
        ...
        if (_claimableRewards > 0) {
            ...
            XEN.safeTransfer(_to, _claimableRewards);
    ...
    function claimMultiple(uint64[] memory _batchIds, address _to) external nonReentrant {
        ...
        if (_totalClaimableRewards > 0) {
            XEN.safeTransfer(_to, _totalClaimableRewards);
        }
    }
    ...
    function record(address _user, uint256 _value) external {
        ...
        if (
            ...
            uint256 _xenAmount;
            uint256 _xenDAOAmount;
            if (xenStaking != IXENStaking(address(0))) {
                (_xenAmount,) = xenStaking.userInfo(_user);
            }
            if (daoStaking != IDaoStaking(address(0))) {
                (_xenDAOAmount,,) = daoStaking.userInfo(_user);
            }

            uint256 _bonusRatio = (_xenAmount + _xenDAOAmount) * uint256(BONUS_PER_TOKEN) / TOKEN_PRECISION;
            uint256 _bonusPoint = _value * _bonusRatio / BONUS_PRECISION;
            uint256 _point = _value + _bonusPoint;
            emit RecordAdded(_user, _value, _xenDAOAmount, _xenAmount, _point, currentBatch);
        }
    }
    ...
    function addReward(uint256 _rewardTokens) external nonReentrant {
        ...
        if (_rewardTokens > 0) {
            XEN.safeTransferFrom(msg.sender, address(this), _rewardTokens);
        }
    ...
    function setXenStaking(address _xenStaking) external onlyOwner {
        require(_xenStaking != address(0), "Invalid address");
        xenStaking = IXENStaking(_xenStaking);
        emit XenStakingSet(_xenStaking);
    }
    ...
    /*================ EVENTS ===================*/
    event XenStakingSet(address _xenStaking);
    ...
    event RecordAdded(
        address _user, uint256 _value, uint256 _daoStaking, uint256 _xenStaking, uint256 _point, uint64 _batchId
    );
