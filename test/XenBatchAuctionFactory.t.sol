...
contract XenBatchAuctionFactoryTest is Test {
    ...
    function setUp() external {
        ...
        // store decimals
        vm.store(Constants.XEN, bytes32(abi.encode(5)), bytes32(abi.encode(uint8(18))));
    ...
    function test_set_auction_admin() external {
        ...
        vm.prank(owner);
        XenBatchAuctionFactory auctionFactory = new XenBatchAuctionFactory(
            treasury,
            admin,
            1 days,
            2e18
    ...
    function createFactory() internal returns (address treasury, XenBatchAuctionFactory factory) {
        ...
        factory = new XenBatchAuctionFactory(
            treasury,
            admin,
            vestingDuration,
            minCeilingPrice
        ...
        AuctionTreasury(treasury).setXENAuctionFactory(address(factory));
        deal(Constants.XEN, treasury, 1_000_000 ether);
        vm.stopPrank();
    }

    function test_create_auction_not_allow_unauthorized() external {
        (, XenBatchAuctionFactory factory) = createFactory();
        vm.prank(eve);
