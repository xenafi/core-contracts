...
contract BatchAuctionTest is Test {
    ...
    function setUp() external {
        XEN = address(new MockERC20("XEN", "", 18));
    ...
    function test_create_auction_should_validate_params() external {
        ...
        check_validate_init_params(
            XEN, USDT, tokenAmount, 1, 10002, 1, 1, 0, admin, treasury, 1 minutes, "start time < current time"
        );

        check_validate_init_params(
            XEN, USDT, tokenAmount, 10000, 9999, 1, 1, 0, admin, treasury, 1 minutes, "end time < start price"
        );

        check_validate_init_params(XEN, USDT, 0, 10000, 20000, 1, 1, 0, admin, treasury, 1 minutes, "total tokens = 0");
    ...
    function test_create_auction_success() external {
        ...
        vm.warp(1000);
        XenBatchAuction auction = new XenBatchAuction(
            XEN,
            USDT,
            10 ether,
            1000,
    ...
    function initAuction() internal returns (XenBatchAuction auction) {
        auction = new XenBatchAuction(
            XEN,
            USDT,
            10_000 ether,
            1100, // start
        ...
        deal(XEN, address(auction), 10_000 ether);
    }

    function test_commit_fail_when_auction_not_live() external {
        vm.warp(1000);
        XenBatchAuction auction = initAuction();
    ...
    function test_claim_success_auction() external {
        ...
        */
        ...
        /* alice claim XEN */
        vm.prank(alice);
        vm.expectEmit(true, true, true, true, XEN);
    ...
    function test_claim_failed_auction() external {
        ...
        vm.prank(admin);
        /* XEN refund to treasury when finalize */
        vm.expectEmit(true, true, true, true, XEN);
    ...
    function test_cancel_auction() external {
        ...
        // refund XEN to treasury when cancel
        vm.prank(admin);
        vm.expectEmit(true, true, true, true, XEN);
        emit Transfer(address(auction), treasury, 10_000 ether);
        auction.cancelAuction();
