...
contract TestReferralRewarder is Test {
    ...
    function test_claim_rewards() external {
        ...
        assertEq(MockERC20(rewarder.XEN()).balanceOf(USER), _claimableRewards);
        rewarder.claimRewards(24, USER, 3, USER_REWARDS, merkleProof);
        assertEq(MockERC20(rewarder.XEN()).balanceOf(USER), USER_REWARDS);
        vm.stopPrank();
    }
    ...
    function test_recover_fund() external {
        vm.startPrank(owner);
        init();
        uint256 _amount = MockERC20(rewarder.XEN()).balanceOf(address(rewarder));
        vm.expectRevert("invalid address");
    ...
    function init() internal {
        MockERC20 xen = new MockERC20("XEN", "XEN", 18);
        ...
        vm.etch(rewarder.XEN(), address(xen).code);
        MockERC20(rewarder.XEN()).mintTo(1_000_000e18, address(rewarder));
